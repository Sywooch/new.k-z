<div class="items-row cols-1 row-0">
    <div class="item column-1">
        <div class="article-meta blog">
            <h2>
                <a href="//<?=$model->fullLink?>">
                    <?=$model->title?>
                </a>
            </h2>

            <ul class="actions">
                <li class="print-icon">
                    <a href="/<?=$model->fullLink?>?tmpl=component&amp;print=1&amp;layout=default&amp;page=" title="Печать" onclick="window.open(this.href,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no'); return false;" rel="nofollow">
                        <img alt="" src="data:image/png;base64,">
                    </a>
                </li>
            </ul>


            <dl class="article-info">
                <dt class="article-info-term">Подробности</dt>

                <dd class="create">
                    Создано <?=Yii::$app->formatter->asDatetime($model->publishDate, 'php:d.m.Y H:i');?>
                </dd>
                <dd class="createdby">
                    Автор: <?=/*isset($model->author) && !empty($model->author) ? $model->author->username : */''?>
                </dd>

            </dl>
        </div>
        <?=$model->textPreview?>
        <div class="jcomments-links">
            <a class="readmore-link" href="/<?=$model->fullLink?>" title="<?=$model->title?>">Подробнее...</a>
            <a class="comments-link" href="/<?=$model->fullLink?>#addcomment" title="Добавить комментарий">
                Добавить комментарий
            </a>
        </div>

        <div class="item-separator"></div>
    </div>

    <span class="row-separator"></span>
</div>